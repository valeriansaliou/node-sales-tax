...
SalesTax.prototype.__readCurrentTaxRates = function(
  ...
  if (typeof (countryTaxRates || {}).before === "object") {
    ...
    for (var beforeDateString in countryTaxRates.before) {
      ...
      // Notice: this stops at the first 'before date' match, which means that \
      //   we do not pick the lowest past date. In other words, we only \
      //   support a single VAT rate change looking at the future.
      if (nowDate < beforeDate) {
        return countryTaxRates.before[beforeDateString];
      }
    }
  }
