  ...
  if (self.enabledTaxNumberValidation === true) {
    ...
    if (cleanTaxNumber) {
      ...
      if (countryCode === "GB") {
        ...
        return Promise.resolve(
          (splitMatch && splitMatch[1]) ? true : false
        );
      }
        ...
        return new Promise(function(resolve, reject) {
          ...
          if (isValid === true && self.enabledTaxNumberFraudCheck === true) {
            ...
            if (splitMatch && splitMatch[1] && splitMatch[2]) {
              // EU fraud check not available? (this only works on Node.js \
              //   platforms due to usage of an underlying 'http' import)
              if (check_fraud_eu_vat === null) {
                return reject(
                  new Error("EU fraud check not available in this environment")
                );
              }
